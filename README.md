...
“卡牌提示词编辑器”左侧只显示卡牌编号和提示词状态，中间显示中文卡图，右侧只有一个提示词框。保存后点击“导出 Codex 任务包”，即可把所有已填写提示词连同对应完整卡牌结构导出给 Codex；Codex 必须逐张同步修改结构化数据、运行代码和回归测试。

提示词编辑器找不到默认校对 JSON 时会弹出文件选择框；发布版不要求 EXE 与工程目录放在一起。

v0.3.36 补全小卡名称、彩色价格标记、技能标记对应颜色和灰色方格强化储备数；编辑器将“文字明确写出的储备上限”与“强化储备数”分开显示，并修正横置卡显示方向。没有可靠标题的卡不再用效果正文冒充名称。
